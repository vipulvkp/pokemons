...
class AddPokemonModal extends React.Component{
    ...
    componentWillMount = () => {
    $('.ui.modal').remove();
    $(".ui.dimmer.visible.active").remove() 
    }
    componentDidMount = () => {
     $('.ui.modal').modal({
         onHide :  this.props.onHideCall
    }).modal('show');
    }
    addField = () =>{
     this.setState(
    ...
    for(let i = 0;i<this.state.numberOfCustomFields;i++){
     customRows.push(
      <div style={{marginTop:"5%",marginBottom:"5%",textAlign:"center"}} key={"custom_"+i} className="ui grid"><div className="eight wide column"><div className="ui input"><input type="text" value={this.state.customFields[i].key} onChange={this.updateCustomFieldInfo(event,i,'key').bind(this)}/></div></div><div className="six wide column"><div className="ui input" style={{float:"left"}}><input type="text" value={this.state.customFields[i].value} onChange={this.updateCustomFieldInfo(event,i,'value').bind(this)}/></div></div><div className="two wide column"><i style={{marginTop:"10%",cursor:"pointer"}} className="large trash alternate icon" onClick={this.removeCustomField.bind(this,i)}></i></div></div>
     )
    }
    return (
        <div>
